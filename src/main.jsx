...
function Orders({ go, orders }) {
  ...
  return <div className="page"><Header title="我的意向单" onBack={()=>go('teachers')} />
    <Tabs activeKey={tab} onChange={setTab}>{Object.entries(groups).map(([k,v])=><Tabs.Tab title={`${k} ${v.length}`} key={k} />)}</Tabs>
    <div className="orders-tools"><SearchBar placeholder="搜索学员姓名" value={query} onChange={setQuery} /><Button onClick={()=>setTeacherOpen(true)}>任课老师 <ChevronDown size={14} /></Button></div>
